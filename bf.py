...
A coluna "Mês" da Lista NÃO é mês do calendário — é seletor de origem:
    ...
    Mês 15  -> encerramento                 : LANCAMENTOCONTABIL INMES = 13
...
termos explícitos de INMES = 13, contando duas vezes. Por isso o guard em
...
# ── Encerramento: LANCAMENTOCONTABIL INMES = 13 ─────────────────────────────
# Zerado em meses intermediários. Mantido para que o mesmo código sirva ao
# fechamento anual sem alteração.
SQL_ENC = f"""
SELECT
    - SUM(CASE WHEN o.INMES = 13 AND o.COCONTACONTABIL = 622920104
           THEN {_SC} ELSE 0 END)
    + SUM(CASE WHEN o.INMES = 13
...
def extrair(conn, mes, ano):
    if mes > 12:
        raise ValueError(
            f"--mes {mes} nao suportado. O seletor 'Mes' da Lista de Equacoes "
            "usa: 0 = abertura, 1 = lancamento ate o mes 12, 13 = lancamento "
            "ate o mes 13, 15 = lancamento NO mes 13. O rotulo 15 NAO e "
            "INMES=15 -- essa particao nao existe no banco. Com mes>12 o range "
            "1..mes incluiria o mes 13, que ja e somado a parte pelo termo de "
            "encerramento, contando duas vezes. E o INMES=14 (87.318 "
            "lancamentos em MIL2025) nao e coberto por nenhuma opcao do "
            "seletor. Use mes<=12 ate definir o encerramento com quem mantem "
            "a Lista.")

    t = query_one(conn, SQL_MOV.format(mes=mes, ano=ano))
    t.update(query_one(conn, SQL_ENC.format(ano=ano)))
    t.update(query_one(conn, SQL_CAIXA.format(mes=mes, ano=ano)))
    ini = query_one(conn, _sql_saldo_ini(ano))
    mov = query_one(conn, _sql_saldo_mov(mes, ano))
    t.update(ini)
    t.update(mov)

    # 1.05 Saldo do Exercício Anterior (a 113829700 entra como SC aqui)
    t["SALDO_ANT_TOT"] = (ini["INI_SD_CAIXA"] + ini["INI_SD_RPPS"]
                          + ini["INI_SD_DEPR"] + ini["INI_SD_VCOMP"]
                          + ini["INI_SD_VTRAN"] + ini["INI_SC_VTCR"])

    # 2.05 Saldo p/ o Exercício Seguinte = abertura + movimento do período.
    # A 113829700 entra como SD aqui (item 2.05.05) — sinal oposto ao anterior.
    t["SALDO_SEG_TOT"] = sum(
        (ini[f"INI_SD_{k}"] + mov[f"MOV_SD_{k}"]) for k, _ in _GRUPOS)
