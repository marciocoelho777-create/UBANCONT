# -*- coding: utf-8 -*-
"""
Cópia de dfc.py — filtro por tipo de agregação de gestão. NÃO mexer no original.
...
Filtro: SQL_DFC já tinha um placeholder {filtro_ug} pronto na cláusula WHERE
principal (aplicado uma única vez, filtra todos os SUM(CASE...) do SELECT de
uma vez, já que é tudo a mesma query sobre LANCAMENTOCONTABIL). Usamos
COGESTAOCONTAB (gestão contabilizante) para casar com o mesmo campo já usado
nos JOINs internos com NOTAEMPENHO. SQL_CAIXA (SALDOCONTABIL) não tem
COGESTAOCONTAB, só COGESTAO — filtro à parte, mesmo padrão do bf_tipoagreg.py.
"""
...
def _filtro_cogestaocontab(cogestao_list):
    if not cogestao_list:
        return ""
    ids = ",".join(str(int(c)) for c in cogestao_list)
    return f"AND o.COGESTAOCONTAB IN ({ids})"


def _qf_caixa(conn, sql, cogestao_list, **fmt):
    s = sql.format(**fmt) if fmt else sql
    if cogestao_list:
        ids = ",".join(str(int(c)) for c in cogestao_list)
        kw = "AND" if "WHERE" in s.upper() else "WHERE"
        s += f"\n  {kw} v.COGESTAO IN ({ids})"
    return query_one(conn, s)

# Tolerância do DFC-01. Alinhada à do cruzamento X2 (cruzamentos.py), que
# avalia exatamente o mesmo GAP_X2. Antes o DFC-01 usava 50,00 e o X2 usava
# 0,05: um gap de R$ 10,00 passaria aqui e reprovaria lá — dois controles
# discordando sobre o mesmo número. Se voltar a aparecer resíduo recorrente,
# mexer nos DOIS, não só neste.
TOL_DFC01 = Decimal("0.05")
...
--   (estas contas NAO usam empenho — filtro direto por conta contabil)
  ...
  + SUM(CASE WHEN o.INMES BETWEEN 1 AND :mes AND o.INDEBITOCREDITO = 'D'
              ...
              AND o.COCONTACONTABIL NOT IN (218810304,218810405)
         THEN o.VALANCAMENTO ELSE 0 END)
  -- Item 1.02.04 da Lista de Equações do DFC. Quinto dígito = esfera:
  -- 1 consolidado, 2 intra, 3 inter. As duas contas "inter" (237130301 e
  -- 237230301) faltavam aqui e no bf.py, e valem ~R$ 7 mil.
  + SUM(CASE WHEN o.INMES BETWEEN 1 AND :mes
...
def extrair(conn, mes, ano, cogestao_list=None):
    """Executa SQL_DFC + SQL_CAIXA, com filtro opcional por cogestao_list."""
    ...
    filtro_ug = _filtro_cogestaocontab(cogestao_list)
    sql = SQL_DFC.format(ano=ano, filtro_ug=filtro_ug).replace(":mes", str(mes))
    cur = conn.cursor()
    cur.execute(sql)
    ...
    if row is None:
        ...
        for c, v in zip(cols, row):
            try:
                ...
                t[c] = D(0)
    t.update(_qf_caixa(conn, SQL_CAIXA, cogestao_list, mes=mes, ano=ano))
    # SUM() Oracle retorna NULL quando nenhuma linha passou pelo filtro de
    # COGESTAO (ex.: tipo sem nenhum lançamento de caixa no período).
    # Substitui todos os None por D(0) para não quebrar a aritmética a seguir.
...
def auditar(conn, mes, ano, cogestao_list=None):
    ...
    # Tolerância: empenhos de exercícios anteriores a 2023 não são capturados
    # pela busca de natureza de despesa (query só alcança MIL2023..MILano).
    # Isso causa um gap residual pequeno e recorrente (~R$ 31 mil em jul/2026).
    # Gap > R$ 100 mil = ERRO real; gap <= R$ 100 mil = ALERTA (resíduo esperado).
    if checa_gap(gap, TOL_DFC01):
        achados.append(achado_ok("DFC","DFC-01",
            "Geracao Liquida (I+II+III) = Variacao do Caixa",
            f"Geracao {ger:,.2f}  =  Variacao {var:,.2f}", valor=ger))
    elif abs(gap) <= Decimal("100000"):
        achados.append(achado_alerta("DFC","DFC-01",
            "Geracao Liquida != Variacao do Caixa (gap residual)",
            f"Gap: {gap:,.2f}  |  Geracao {ger:,.2f}  vs  Var.Caixa {var:,.2f}  "
            f"-- gap pequeno e esperado: empenhos pre-2023 nao capturados pela "
            f"busca de natureza de despesa (alcance: MIL2023..MIL{ano})",
            valor=gap))
    else:
    ...
    for nome, val, cod in [("Caixa Inicial", t.get("CAIXA_INI",D(0)), "DFC-02a"),
        ...
        if val < 0:
            achados.append(achado_erro("DFC", cod, f"{nome} negativo",
                                       f"{val:,.2f}", valor=val))
        else:
            achados.append(achado_ok("DFC", cod, f"{nome} positivo",
                                     f"{val:,.2f}", valor=val))
