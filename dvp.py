...
891XXXXXX, que o extrair() já trazia e que ninguém usava.

AJUSTE 14/08/2026: a primeira versão lia a 891 com INMES 1..mes e disparou
ERRO de R$ 15 bi em jul/2026 — a conta tinha R$ 256.016,48 de movimentação
avulsa, e o controle comparou isso com o resultado de -15,02 bi. Grandezas
não comparáveis. A leitura passou para INMES 13..15, que é quando o
encerramento de fato ocorre. Fora do fechamento o controle vira INFO e
delega a validação aos cruzamentos X6 (DMPL) e R1a (BP), em vez de fingir
que verificou algo.
"""
...
SELECT
    ...
    -- INMES 13..15, NÃO 1..mes: o resultado só é encerrado contra a 891 no
    -- fechamento do exercício. Lendo 1..mes captava movimentação avulsa da
    -- conta (R$ 256.016,48 em jul/2026) e o controle comparava esse valor
    -- com o resultado de -15,02 bi, produzindo um "erro" de 15 bi que era
    -- só a subtração de duas grandezas não comparáveis.
    SUM(CASE WHEN v.INMES BETWEEN 13 AND 15
              AND v.COCONTACONTABIL BETWEEN 891000000 AND 891999999
         THEN v.VACREDITO - v.VADEBITO ELSE 0 END)   AS RESULTADO_ENCERRAMENTO,

    -- Movimentação da 891 no período corrente — informativo, não é o
    -- encerramento. Serve para flagrar lançamento indevido na conta.
    SUM(CASE WHEN v.INMES BETWEEN 1 AND {mes}
              AND v.COCONTACONTABIL BETWEEN 891000000 AND 891999999
         THEN v.VACREDITO - v.VADEBITO ELSE 0 END)   AS MOV_891_PERIODO
...
def auditar(conn, mes: int, ano: int) -> tuple[list[Achado], dict]:
    ...
    if checa_gap(enc):
        mov = t.get("MOV_891_PERIODO", D(0))
        extra = ("" if checa_gap(mov) else
                 f"  (há {mov:,.2f} de movimentação avulsa na 891 no período — "
                 f"não é encerramento, mas vale verificar a origem)")
        achados.append(achado_info("DVP", "DVP-01",
            "Resultado Patrimonial (exercício ainda não encerrado)",
            f"VPA {vpa:,.2f}  −  VPD {vpd:,.2f}  =  {resultado:,.2f}  — a conta "
            f"891XXXXXX só recebe o encerramento em INMES 13..15, que ainda não "
            f"ocorreu; a validação do resultado fica com os cruzamentos X6 "
            f"(DMPL) e R1a (BP){extra}", valor=resultado))
    else:
        gap = resultado - enc
        if checa_gap(gap):
